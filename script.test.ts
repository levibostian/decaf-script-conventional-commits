...
Deno.test("exits early when no commits since last release", async () => {
  ...
  assertEquals(
    result.stdout.includes("No commits since the last release"),
    true,
  )
})
...
Deno.test("first release: create a release only when qualifying commits exist", async () => {
  const tempFile = setupTestEnv({
    gitCurrentBranch: "main",
    gitRepoOwner: "test-owner",
    gitRepoName: "test-repo",
    testMode: false,
    // A commit that qualifies for a release. Expect this to cause a release to be created.
    gitCommitsSinceLastRelease: [
      {
        sha: "abc001",
