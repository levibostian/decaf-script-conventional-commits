...
if (input.gitCommitsSinceLastRelease.length === 0) {
  console.log(
    "No commits since the last release. There are no commits to analyze and no new version will be released. Exiting...",
  )
  Deno.exit(0)
}

console.log(
  "I'm going to look at each commit and figure out if any of them are important enough to trigger a new release.",
)
console.log(
...
const versionBumpsForEachCommit: ("major" | "minor" | "patch")[] = input
  .gitCommitsSinceLastRelease.map((commit) => {
    const abbreviatedCommitTitle = commit.title.length > 50 ? commit.title.substring(0, 47) + "..." : commit.title.padEnd(50, " ")
...
if (versionBumpsForEachCommit.length === 0) {
  console.log(
    `None of these commits are important enough to trigger a release. I only look for commits that begin with:`,
  )
  console.log(`1. "feat:" - indicates a new or modified feature`)
  console.log(`2. "fix:" - indicating a bug fix`)
  console.log(
    `3. "feat!:" (the ! character before colon) - indicating a breaking change`,
  )
...
if (!lastReleaseSemanticVersion) {
  console.error(
    `Last release version "${lastReleaseVersion}" is not a valid semantic version. It cannot be parsed. Therefore, the next release version cannot be determined.`,
  )
...
const nextVersion = semver.format(
